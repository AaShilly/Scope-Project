...
function interrupter(interruptingWord) {
  ...
  // Utilize closure to acess outer function argument interruptingword
  // Splits the sentence passed through to the inner function into an array
  // returns the array joined back together with interruting word inbetween
  // each word
  return (sentence) => {
    let interrupted = sentence.split(" ");
    return interrupted.join(` ${interruptingWord} `);
  }
}
